...
export default function Sidebar({ onCapture, onOpenTutorial, inboxCount, emailCount }: Props) {
  return (
    <aside className="flex h-full w-60 shrink-0 flex-col overflow-y-hidden border-r border-dusk-light bg-paper">
      ...
      <div className="space-y-0.5 border-t border-dusk-light px-3 pb-5 pt-2">
        <button
          ...
          className="flex w-full items-center gap-2.5 rounded-md px-3 py-2 text-left text-sm text-dusk transition hover:bg-fog"
        >
          <HelpCircle size={16} />
          Take the tour
        </button>
