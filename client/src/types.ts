...
export interface EmailItem extends BaseRecord {
  subject: string;
  rawContent: string;
  summary?: string;
  priority: Priority;
  ...
  links?: EmailLink[]; // quick links — portal, tracking page, doc, etc — one click to get back there
  archived?: boolean; // handled / dealt with, hidden from the main view by default
  eventId?: ID; // linked Calendar deadline event, kept in sync automatically
  taskId?: ID; // linked Planner task, created on demand
}

export type TrashCollection = "inbox" | "tasks" | "projects" | "documents" | "notes" | "events" | "emails";
...
export type SearchResultType = "task" | "project" | "document" | "note" | "email";
