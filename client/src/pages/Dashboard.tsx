...
import { CheckCircle2, Circle, ArrowRight, FolderKanban, Briefcase, Mail } from "lucide-react";
import { api, todayISO } from "../lib/api";
import type { Task, Project, CalendarEvent, Settings, EmailItem } from "../types";
...
export default function Dashboard() {
  ...
  const emailsNeedingAttention = emails
    .filter((e) => {
          ...
          <Card className="p-6">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="flex items-center gap-2 font-display text-lg text-ink">
                <Mail size={16} /> Emails needing attention
              </h2>
              <Link to="/email" className="flex items-center gap-1 text-xs text-moss hover:underline">
            ...
            <div className="space-y-2.5">
              {emailsNeedingAttention.map((e) => (
                <Link
                  key={e.id}
                  to={`/email?email=${e.id}`}
                  className="flex items-center gap-3 rounded-md px-2 py-1.5 text-sm hover:bg-fog"
                >
                  <PriorityDot priority={e.priority} />
                  <span className="flex-1 truncate text-ink-soft">{e.subject}</span>
                  {e.deadline && (
                    <Pill tone="ember">
                      {new Date(e.deadline + "T00:00:00").toLocaleDateString(undefined, {
                        month: "short",
                        day: "numeric",
                      })}
                    </Pill>
