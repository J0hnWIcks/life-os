...
import { Plus, X, Download, Upload, Check, Bell, BellOff, Link2, Unlink, HelpCircle } from "lucide-react";
...
export default function Settings() {
  const { openTutorial } = useOutletContext<{ openTutorial?: () => void }>();
    ...
    <div className="mx-auto max-w-2xl px-4 py-8 sm:px-8">
      ...
      <Card className="mb-5 p-5">
        <h2 className="mb-1 flex items-center gap-2 font-display text-base text-ink">
          <HelpCircle size={16} /> New here?
        </h2>
        <p className="mb-3 text-xs text-dusk">
          Replay the quick walkthrough of Life OS — capture, email, planning, projects, and how it all fits
          together.
        </p>
        <button
          onClick={() => openTutorial?.()}
          className="flex items-center gap-1.5 rounded-md border border-dusk-light px-3.5 py-2 text-sm font-medium text-ink-soft hover:bg-fog"
        >
