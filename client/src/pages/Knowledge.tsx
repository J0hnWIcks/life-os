...
export default function Knowledge() {
  ...
  const isPreview = selected?.viewMode === "preview";

  async function createNote() {
    const created = await api.notes.create({ title: "Untitled note", content: "", tags: [], viewMode: "edit" });
    load();
            ...
            <Card className="p-5">
              <div className="mb-3 flex items-start justify-between gap-3">
                ...
                <div className="flex shrink-0 items-center gap-1">
                  <button
                    onClick={() => patchSelected({ viewMode: isPreview ? "edit" : "preview" })}
                    title={isPreview ? "Switch to edit" : "Switch to read-only preview"}
                    className={`rounded-md p-1.5 ${isPreview ? "bg-moss-light text-moss" : "text-dusk hover:bg-fog"}`}
                  >
                    {isPreview ? <Pencil size={15} /> : <Eye size={15} />}
                  </button>
                  <button
