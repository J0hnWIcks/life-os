...
import { Tags as TagsIcon, FileText, BrainCircuit, Mail } from "lucide-react";
import { api } from "../lib/api";
import type { DocumentItem, KnowledgeNote, EmailItem } from "../types";
...
export default function Tags() {
  ...
  useEffect(() => {
    ...
    api.emails.list().then(setEmails);
  }, []);
  ...
  const tagCounts = useMemo(() => {
    ...
    emails.forEach((e) => e.tags.forEach((t) => (counts[t] = (counts[t] || 0) + 1)));
    return counts;
  }, [docs, notes, emails]);
  ...
  const matchingEmails = selectedTag ? emails.filter((e) => e.tags.includes(selectedTag)) : [];

  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-8">
      <PageHeader
        title="Tags"
        subtitle="One place to browse everything you've tagged, across Documents, the Knowledge Base, and Email."
      />
      ...
      {sortedTags.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
          ...
          <div className="md:col-span-2 space-y-5">
            {matchingDocs.length === 0 && matchingNotes.length === 0 && matchingEmails.length === 0 ? (
                    ...
                    <ul className="space-y-1.5">
                      {matchingEmails.map((e) => (
                        <li key={e.id}>
                          ...
                          >
                            <span className="flex-1 truncate">{e.subject || "(untitled email)"}</span>
                            <Pill tone="neutral">{e.priority}</Pill>
                          </Link>
                        </li>
                      ))}
                    </ul>
