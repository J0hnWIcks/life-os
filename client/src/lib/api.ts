...
export const api = {
  ...
  emails: makeCollection<EmailItem>("emails"),
  ...
  calendar: {
    restoreHolidays: () => request<{ seeded: boolean }>("/calendar/seed-holidays", { method: "POST" }),
  },
  ...
  profiles: {
    list: () => request<{ profiles: Profile[]; activeId: string }>("/profiles"),
    create: (name: string) => request<Profile & { activeId: string }>("/profiles", { method: "POST", body: JSON.stringify({ name }) }),
    rename: (id: string, name: string) => request<Profile>(`/profiles/${id}`, { method: "PUT", body: JSON.stringify({ name }) }),
    remove: (id: string) => request<{ deleted: boolean; activeId: string }>(`/profiles/${id}`, { method: "DELETE" }),
    activate: (id: string) => request<{ activeId: string }>(`/profiles/${id}/activate`, { method: "POST" }),
  },
};
