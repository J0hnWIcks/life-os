        ...
        <div className="md:col-span-2 space-y-5">
          ...
          <Card className="p-5">
            ...
            <p className="mb-3 text-xs text-dusk">
              Project tasks live here and stay out of your Daily Planner by default. Use the{" "}
              <CalendarPlus size={11} className="inline align-text-bottom" /> icon on a task to show it in the
              planner for a specific day.
            </p>
                ...
                {tasks.map((t) => (
                  <li key={t.id} className="group rounded-md px-2 py-1.5 hover:bg-fog">
                    <div className="flex items-center gap-2.5 text-sm">
                      ...
                      {t.date ? (
                        <button
                          onClick={() => unscheduleTask(t)}
                          title={`Showing in Daily Planner on ${t.date}. Click to hide it again.`}
                          className="flex shrink-0 items-center gap-1 rounded-full border border-moss/40 bg-moss-light px-2 py-0.5 text-[11px] text-moss"
                        >
                          <CalendarPlus size={11} /> {t.date}
                          <X size={10} />
                        </button>
                      ) : (
                        <button
                          onClick={() => setSchedulingTaskId(schedulingTaskId === t.id ? null : t.id)}
                          title="Show in Daily Planner"
