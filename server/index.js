...
const { seedHolidays } = require("./holidays");
...
// Resolved fresh on every use (not cached at module load) since the active
// profile — and therefore the correct uploads folder — can change at runtime.
function getUploadsDir() {
  const dir = path.join(store.getActiveProfileDir(), "uploads");
...
const TRASHABLE_COLLECTIONS = ["inbox", "tasks", "projects", "documents", "notes", "events", "emails"];
...
function trashLabel(collection, item) {
  ...
  if (collection === "emails") return item.subject || "(untitled email)";
...
function registerArrayCollection(name, hooks = {}) {
  ...
  app.put(`${base}/:id`, (req, res) => {
    ...
    const patch = hooks.beforeUpdate ? hooks.beforeUpdate(existing, req.body || {}) : req.body || {};
    const item = store.update(name, req.params.id, patch);
    res.json(decorateAll([item])[0]);
  });
  ...
  app.delete(`${base}/:id`, (req, res) => {
    const existing = store.getById(name, req.params.id);
    if (!existing) return res.status(404).json({ error: "not_found" });
    if (hooks.beforeDelete) hooks.beforeDelete(existing);
    if (trashable) {
...
registerArrayCollection("tasks", {
  ...
  supportsSeries: true,
  // `date` (used by the Daily Planner) is optional for project tasks, so we
  // can't rely on it to say when a task was completed. Track that
  // separately here — this is what lets Weekly Review / Analytics correctly
  // attribute a completed project task to the week it was actually done in.
  beforeUpdate: (existing, patch) => {
    if (typeof patch.done === "boolean" && patch.done !== existing.done) {
      return { ...patch, completedAt: patch.done ? new Date().toISOString() : null };
    }
    return patch;
  },
});
...
// --- Emails: parsed college-inbox items, kept in sync with Calendar & Planner ---
//
// An email's deadline is mirrored into a "deadline"-type Calendar event so it
// shows up alongside everything else on the Calendar tab without the user
// having to enter it twice. The link is tracked via `eventId` on the email:
//  - creating an email with a deadline creates the event
//  - editing the deadline (or clearing it) updates/removes the event
//  - deleting the email removes the event too
function syncEmailDeadlineEvent(email, previous) {
  const hadDeadline = !!(previous && previous.deadline);
  const hasDeadline = !!email.deadline;
  const title = email.subject ? `Due: ${email.subject}` : "Email deadline";

  if (!hasDeadline) {
    if (hadDeadline && previous.eventId) {
      store.purge("events", previous.eventId);
      return { eventId: undefined };
    }
    return {};
  }

  const existingEventId = previous && previous.eventId;
  const existingEvent = existingEventId ? store.getById("events", existingEventId) : null;

  if (existingEvent) {
    store.update("events", existingEvent.id, { title, date: email.deadline, time: email.deadlineTime, type: "deadline" });
...
registerArrayCollection("emails", {
  onCreate: (req, res) => {
    const payload = req.body || {};
    const created = store.create("emails", {
    ...
    const { eventId } = syncEmailDeadlineEvent(created, null);
    const final = eventId ? store.update("emails", created.id, { eventId }) : created;
    res.status(201).json(final);
  },
  beforeUpdate: (existing, patch) => {
    const merged = { ...existing, ...patch };
    const result = syncEmailDeadlineEvent(merged, existing);
    if (Object.prototype.hasOwnProperty.call(result, "eventId")) {
      return { ...patch, eventId: result.eventId };
    }
    return patch;
  },
  beforeDelete: (existing) => {
    if (existing.eventId) store.purge("events", existing.eventId);
  },
});
...
app.post("/api/documents/:id/upload", upload.single("file"), (req, res) => {
  ...
  if (doc.fileName) {
    const oldPath = path.join(getUploadsDir(), doc.fileName);
...
const SETTINGS_DEFAULTS = {
  ...
  calendarColors: { event: "moss", deadline: "ember", recurring: "sky", birthday: "plum" },
  holidaysSeeded: false,
};
...
// --- Profiles: separate data folders so different people (or different
// contexts for the same person) never mix data on one install. ---

app.get("/api/profiles", (req, res) => {
  res.json({ profiles: store.listProfiles(), activeId: store.getActiveProfileId() });
});
...
app.post("/api/profiles", (req, res) => {
  ...
  store.setObject("settings", { ...SETTINGS_DEFAULTS, ...settings, holidaysSeeded: true });
  res.json({ ...profile, activeId: store.getActiveProfileId() });
});

app.put("/api/profiles/:id", (req, res) => {
  const name = (req.body && req.body.name ? String(req.body.name) : "").trim();
  if (!name) return res.status(400).json({ error: "name_required" });
  const profile = store.renameProfile(req.params.id, name);
...
app.delete("/api/profiles/:id", (req, res) => {
  try {
    ...
    res.json({ deleted: true, activeId: store.getActiveProfileId() });
  } catch (err) {
    if (err.code === "cannot_delete_last_profile") {
...
app.post("/api/profiles/:id/activate", (req, res) => {
  const ok = store.setActiveProfile(req.params.id);
  if (!ok) return res.status(404).json({ error: "not_found" });
  res.json({ activeId: store.getActiveProfileId() });
});
...
app.get("/api/analytics/weekly", (req, res) => {
  ...
  // "Scheduled" tasks: whatever was planned in the Daily Planner for the
  // week, regardless of when (or whether) they were finished.
  const weekTasks = tasks.filter((t) => t.date && t.date >= weekStart && t.date <= weekEnd);

  // "Completed" tasks: attributed to the day they were actually finished
  // (completedAt), not the day they were scheduled for — this is what lets
  // a project task with no planner date still show up in the week it was
  // done. Tasks completed before this feature existed have no completedAt;
  // fall back to their planner date so old data doesn't just disappear.
  const completedInWeek = tasks.filter((t) => {
    if (!t.done) return false;
    const completedDay = (t.completedAt || t.date || "").slice(0, 10);
    return completedDay >= weekStart && completedDay <= weekEnd;
  });

  function dayOf(t) {
    return (t.completedAt || t.date || "").slice(0, 10);
  }
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const dayTasks = tasks.filter((t) => t.date === iso);
    const dayCompleted = completedInWeek.filter((t) => dayOf(t) === iso);
    const log = dailyLogs[iso];
    days.push({
      date: iso,
      total: dayTasks.length,
      completed: dayTasks.filter((t) => t.done).length,
      completedTasks: dayCompleted.map((t) => ({
        id: t.id,
        title: t.title,
  ...
  // Every project touched this week — whether that's a task scheduled in
  // the planner or a task completed straight from the Projects hub.
  const projectIdsTouched = new Set(completedInWeek.map((t) => t.projectId).filter(Boolean));
  const projectsTouched = projects
    ...
    .map((p) => {
      ...
      const done = pTasks.filter((t) => t.done).length;
      const progress = pTasks.length ? Math.round((done / pTasks.length) * 100) : p.progress || 0;
      const completedThisWeekTasks = completedInWeek.filter((t) => t.projectId === p.id);
      return {
        id: p.id,
        name: p.name,
        progress,
        completedThisWeek: completedThisWeekTasks.length,
        completedTaskTitles: completedThisWeekTasks.map((t) => t.title),
      };
    });
  ...
  res.json({
    ...
    weekEnd,
    days,
    totals: { total: weekTasks.length, completed: completedInWeek.length },
    goalsSet,
    reflections,
    completedTasks: completedInWeek.map((t) => ({
      id: t.id,
      title: t.title,
      date: dayOf(t),
      projectId: t.projectId,
...
app.get("/api/search", (req, res) => {
  ...
  store.list("emails").forEach((e) => {
    const tagMatch = (e.tags || []).some((t) => t.toLowerCase().includes(q));
    const contactMatch = [e.contact?.name, e.contact?.email].filter(Boolean).some((v) => v.toLowerCase().includes(q));
    if (
      ...
      results.push({ type: "email", id: e.id, title: e.subject || "(untitled email)", subtitle: e.contact?.name || "Email" });
    }
  });
...
app.post("/api/calendar/seed-holidays", (req, res) => {
  seedHolidays(store);
  const settings = store.getObject("settings", SETTINGS_DEFAULTS);
  store.setObject("settings", { ...SETTINGS_DEFAULTS, ...settings, holidaysSeeded: true });
